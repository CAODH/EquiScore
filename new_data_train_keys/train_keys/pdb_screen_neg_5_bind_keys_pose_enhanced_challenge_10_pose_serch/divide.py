...
valid_keys_poses = get_part_data(cross_decoys_dir_pose,poses,fast_num=100,active_names = None)
...
############### cluster all ###########
all_cluster_pose =glob.glob('/home/caoduanhua/score_function/data/D-PDBbind_PDBscreen/data_agument_2a_culter_pocket/*')
all_cluster_bind = [i for i in all_cluster_pose if '-' not in i]
all_cluster_screen = [i for i in all_cluster_pose if '-'  in i]
valid_keys += all_cluster_bind
######## cross decoys###################
...
active_pros_screen_crossdecoy = set([v.split('/')[-1].replace('_active_0','') for v in valid_keys_screen])

############################ add enhanced poses ### xxxx_xxx_active_0
...
# screen_poses = [i.split('_')[0] + '_china(a)_' +  i.split('_')[2] for i in screen_poses]
valid_screen_poses = get_part_data_screen_pose(cross_decoys_dir_screen_pose,screen_poses,fast_num=100,active_names =None)
# valid_screen_poses= [valid_screen_poses]
valid_keys_screen += valid_screen_poses
print('pose enhanced screen samples: ',len(valid_screen_poses))
###########################################   add cluster pose ########### xxxx-xxx_active_augment_0
# all_cluster_screen = [i.replace('-','_chain(a)_') for i in all_cluster_screen]
print('pose cluster screen samples: ',len(all_cluster_screen))
valid_keys_screen += all_cluster_screen

############################################

print('screen pose pros: ',len(set([i.split('/')[-1].split('_')[0] for i in valid_keys_screen])))
# active_pros_screen = [i.split('_')[0] + '_' +  i.split('_')[2] for i in valid_keys_screen]
# print('all valid keys in screen ',len(valid_keys_screen))
# active_pros_screen = set([v.split('/')[-1].split('_')[0] + '_' + v.split('/')[-1].split('_')[1] + ' _' + v.split('/')[-1].split('_')[2] for v in valid_keys_screen]) # xxxx_chain_xxx_active_0
# print('#######################',valid_keys_screen[-1])
...
valid_keys_screen += get_part_data_screen(cross_decoys_dir_screen,cross_decoys_screen,fast_num=10,active_names = active_pros_screen_crossdecoy )
print('len of decoy in screen :     #######',len(get_part_data_screen(cross_decoys_dir_screen,cross_decoys_screen,fast_num=10,active_names = active_pros_screen_crossdecoy )))
...
active_pros_screen = set([v.split('_')[0] + '_' + v.split('_')[2] for v in active_pros_screen_crossdecoy])
