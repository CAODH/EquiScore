...
def run(local_rank,args,*more_args,**kwargs):
    ...
    # if you docking result have different pose number, you can use the following function

    # if one compound has only one pose, you can use the following function
    ...
    if args.test_mode == 'multi_pose':
        '''
        if one compound has multi pose, you can use the following function
        ...
        BE CAREFUL: this function may fail depending on the result filname you set ,i will fix it in the future
        '''  
