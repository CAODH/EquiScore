...
class EncoderLayer(nn.Module):
    def __init__(self, hidden_size, ffn_size, dropout_rate, attention_dropout_rate, head_size,norm_type):
        super(EncoderLayer, self).__init__()

        self.self_attention_norm = GetGrpahNorm(hidden_size,norm_type)
        self.self_attention = MultiHeadAttention(hidden_size, attention_dropout_rate, head_size)
        self.self_attention_dropout = nn.Dropout(attention_dropout_rate)

        self.ffn_norm = GetGrpahNorm(hidden_size,norm_type)
...
#参数里默认值0.1，8
    ...
    if args.fundation_model == 'graphformer':
        return EncoderLayer(args.n_out_feature, args.ffn_size, args.dropout_rate, args.attention_dropout_rate, args.head_size,args.norm_type)