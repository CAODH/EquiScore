...
class gnn_edge(torch.nn.Module):
    ...
    def GetAttnBias(self,rel_pos,attn_bias,all_rel_pos_3d_l):
        # graph_attn_bias
    ...
    def embede_single_graph(self, data):
        ...
        attn_bias_2 = self.GetAttnBias(data.rel_pos_1,data.attn_bias,data.all_rel_pos_3d)
        edge_input = self.edge_encoder(data.attn_edge_type_1).mean(-2)#[bs,n,n,edge_dim]
    ...
    def forward(self,A2_limit,sample):
        ...
        if self.args.embed_graph == 'single_graph':
            c_hs = self.embede_single_graph(sample)
